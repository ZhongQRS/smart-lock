...
void ShowErrMessage(u8 ensure)
{
	...
	{
		OLED_Clear();
		OLED_ShowString(8,0,"AS608 Erro!");  
		delay_ms(2000);
	}
}
...
u16 GET_NUM(void)
{
	...
	while(1)
	{
		...
		if(strcmp((char *)p,"C")==0)//重输
		{
			memset(buf,0,10);
			OLED_ShowString(40,3,(u8 *)"      ");//清屏密码
		}
	...
	while(1)
	{
		switch (processnum)
		{
			case 0:
				i++;
				OLED_Clear();
				OLED_ShowString(8,0,"Please press the fingerprint");
				//printf("请按指纹\r\n");
				ensure=PS_GetImage();
				if(ensure==0x00) 
				{
					...
					{
						OLED_Clear();
						OLED_ShowString(8,0,"First success");
						//printf("指纹正常\r\n");
						i=0;
						processnum=1;//跳到第二步						
			...
			case 1:
				i++;
				OLED_Clear();
				OLED_ShowString(8,0,"Please press again");
				//printf("请按再按一次指纹\r\n");
				ensure=PS_GetImage();
				if(ensure==0x00) 
				{
					...
					{
						OLED_Clear();
						OLED_ShowString(8,0,"Secondary success");
						//printf("指纹正常\r\n");
						i=0;
						processnum=2;//跳到第三步
					}else ShowErrMessage(ensure);	
			...
			case 2:
				OLED_Clear();
				OLED_ShowString(8,0,"Comparing");
				//printf("对比两次指纹\r\n");
				ensure=PS_Match();
				if(ensure==0x00) 
				{
					OLED_Clear();
					OLED_ShowString(8,0,"Compare success");
					//printf("对比成功,两次指纹一样\r\n");
					processnum=3;//跳到第四步
				}
				else 
				{
					OLED_Clear();
					OLED_ShowString(8,0,"Compare fail");
					//printf("对比失败，请重新录入指纹\r\n");
			...
			case 3:
				OLED_Clear();
				OLED_ShowString(8,0,"Generate fingerprint module");
				//printf("生成指纹模板\r\n");
				ensure=PS_RegModel();
				if(ensure==0x00) 
				{
					OLED_Clear();
					OLED_ShowString(8,0,"Generate success");
					//printf("生成指纹模板成功\r\n");
					processnum=4;//跳到第五步
			...
			case 4:	
				OLED_Clear();
				OLED_ShowString(8,0,"Please enter ID");
				OLED_ShowString(8,2,"0=< ID <=299");
				...
				{			
					OLED_Clear();
					OLED_ShowString(8,0,"Entered successfully");
					//printf("录入指纹成功\r\n");
					PS_ValidTempleteNum(&ValidN);//读库指纹个数
					OLED_ShowString(8,2,"Capacity");
					OLED_ShowNum(8,4,AS608Para.PS_max-ValidN,5,16);
